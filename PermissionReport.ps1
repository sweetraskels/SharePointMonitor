...
Function Generate-PermissionRpt()
{
    ...
    "</table>
<b>Site Collection Administrators</b>" >> $OutputReport
    ...
    foreach($Web in $Site.AllWebs)
    {
        ...
        {
            "</table>
<hr> <h3>Site: <a name='$($Web.Title.ToLower())' href='$($web.URL)' target='_blank'>$($Web.Title)</a> is using Unique Permissions. </h3>" >> $OutputReport
        }
        else
        {
            "</table>
<hr> <h3>Site: <a name='$($Web.Title.ToLower())' href='$($web.URL)' target='_blank'>$($Web.Title)</a> is Inheriting Permissions from its Parent Site.</h3>" >> $OutputReport
        }
     ...
     #********  Check All List's Permissions ********/
        foreach($List in $Web.lists)
        {
            ...
            {
                "</table>
<b>List: [ $($List.Title) ] at <a href='$($List.ParentWeb.Url)/$($List.RootFolder.Url)'>$($List.ParentWeb.Url)/$($List.RootFolder.Url)</a> is using Unique Permissions.</b><table class='altrowstable' id='alternatecolor' cellpadding='5px'><tr>" >> $OutputReport
                "<th>Users/Groups </th><th> Type </th><th> User Name </th><th> Permissions</th></tr>" >> $OutputReport
            ...
            if($ScanFolders -eq $True)
            {
                ...
                if($UniqueFolders)
                {
                    ...
                    foreach($folder in $UniqueFolders)
                    {
                        ...
                        "
<b>Folder: <a href='$($FolderURL)' target='_blank'>$($Folder.Title)</a> is using Unique Permissions.</b><table class='altrowstable' id='alternatecolor' cellpadding='5px'><tr>" >> $OutputReport
                        "<th>Users/Groups </th><th> Type </th><th> User Name </th><th> Permissions</th></tr>" >> $OutputReport
            ...
            if($ScanItemLevel -eq $True)
            {
                ...
                if($UniqueItems)
                {
                    ...
                    foreach($Item in $UniqueItems)
                    {
                        ...
                        "
<b>Item: <a target='_blank' href='$($ItemURL)?ID=$($Item.ID)'>$($ItemTitle)</a> in list/library <a href='$($List.ParentWeb.Url)/$($List.RootFolder.Url)'>$($List.Title) </a> is using Unique Permissions.</b><table class='altrowstable' id='alternatecolor' cellpadding='5px'><tr>" >> $OutputReport
                        "<th>Users/Groups </th><th> Type </th><th> User Name </th><th> Permissions</th></tr>" >> $OutputReport
...
$Subject = "Site Permission Inventory Report as on: "+$ReportDate 
$Body = Get-Content $OutputReport -Raw
#Send Email
...
$OutputReport = "PermissionRpt.htm"
$ScanFolders=$False
$ScanItemLevel=$False

$allwebapps=Get-SPWebApplication
foreach($webapp in $allwebapps)
Generate-PermissionRpt $webapp.url $OutputReport $ScanFolders $ScanItemLevel
}
